function [U,V,iteration] = PS1_fcm(X,c,F,b,alpha)
% PS1_fcm:Partial Supervision fcm by liyang @BNU Math 315
...
while(1)
    ...
    for i = 1:c
        for k = 1:N
            fai(i,k) = U(i,k)^2+alpha*(U(i,k)-F(i,k)*b(k))^2;
        end
    end
    for s = 1:c
        temp_numerator = 0;
    ...
    for s = 1:c
        for t = 1:N
            ...
            temp_numerator = 1+alpha*(1-b(t)*sum(F(:,t)));
            new_U(s,t) = (1+alpha)^(-1) * (temp_numerator / temp_denominator + alpha*F(s,t)*b(t));
        end
    end
